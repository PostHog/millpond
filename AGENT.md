...
### Maintenance tooling: pitfalls

This area has a long tail of subtle bugs that all share a shape: code that looks right against simple inputs and silently misbehaves on production-realistic ones. Before changing anything in `tools/maintenance.py`, `tools/maintenance.sql`, or `tools/justfile`, walk this checklist. Most items trace to a real regression caught in PR #49 review.

- **libpq vs SQL string-literal grammar are different parsers.** A connstring inside `ATTACH 'postgres:...'` traverses both: the outer Postgres SQL parser (uses `''` for embedded quotes, governed by `standard_conforming_strings`) and the inner libpq connstring parser (uses `\'` and `\\`, *not* affected by `standard_conforming_strings`). Apply each escape at its own layer. Tests with alphanumeric-only credentials hide this — the in-tree end-to-end check uses a `postgres` container with `POSTGRES_PASSWORD=O'Brien` to force both parsers to participate.
- **Live filter on every catalog gate.** `ducklake_data_file` and `ducklake_delete_file` carry historical entries with non-null `end_snapshot`. Any cross-table count query that doesn't filter `WHERE end_snapshot IS NULL` will see expired rows as live and block valid operations. Mirror the `compact` subcommand's filter on every gate query.
- **Two path forms always (quirk r1).** The same physical file can appear in catalog `path` columns as either an absolute `s3://...` URI or a key relative to `DUCKLAKE_DATA_PATH`, and both forms can coexist in the same column. Any cross-source comparison must normalize first. The current scheme: pass-through if `path LIKE 's3://%' OR path LIKE '/%'`, else `rtrim(data_path, '/') || '/' || path`.
- **`rtrim` trailing slashes.** `s3://bucket/lake/data/` is a common operator-supplied form. `data_path || '/' || key` produces `.../data//key`, which never matches the absolute form. Always `rtrim(data_path, '/')` before concatenating.
- **Counts from operation return values, not queue-depth deltas.** The advisory lock only mutexes maintenance invocations; arbitrary writers (millpond ingest pods) can change the queue mid-run. A `before - after` delta is racy and can go negative. Use `len(result)` from `ducklake_cleanup_old_files` instead.
- **`tools/maintenance.sql` is verbatim.** Both `connect()` and `just shell` (`.read` meta-command) execute the file as-is. No Python templating, no placeholders. References to `__ducklake_metadata_lake` are literal and the `TestSchemaConsistency` unit test pins the alignment with `METADATA_SCHEMA`.
- **`CREATE MACRO` evaluates literal `glob('s3://...')` eagerly.** A no-arg macro with a baked-in S3 path will S3-LIST on every connect, including from subcommands that don't need orphans. Macros that need a path take it as a parameter.
- **Dry-run runs the gates.** `heal-orphans --dry-run` and `fsck --dry-run` execute the B1/B3 gates so a real run that would abort surfaces in the dry-run output. Conversely, the cleanup throughput log is *suppressed* on dry-run — preview rows from `ducklake_cleanup_old_files` would falsely claim processed work.
- **Lock ordering: take it before materializing state.** Any gated procedure that scans the queue into a temp table and then takes the lock leaves a race window. Lock first, then snapshot, then run gates, then mutate.
- **Test both path forms with the right shapes.** Local filesystem paths (`/tmp/...`) exercise the `LIKE '/%'` clause, *not* the `LIKE 's3://%'` clause. Both need explicit fixtures. Bucket-relative keys are relative to `DUCKLAKE_DATA_PATH` (e.g. `main/events/.../X.parquet`), not to the bucket root — `data_path + '/' + relative_key` must reproduce the absolute form.
- **Justfile string interpolations are SQL.** Every value that lands inside `_setup` and gets executed is SQL. Two layers of escaping apply: libpq for connstring values (`replace("\\", "\\\\")` then `replace("'", "\\'")`) and SQL for everything that ends up inside an outer SQL string literal (`replace("'", "''")`). For unquoted SQL booleans (`USE_SSL <bool>`) there's no escape that helps — whitelist via `if/else error()` instead.
- **Doc shapes drift after refactors.** Whenever you change a structured log line, error message, CLI flag default, or env-var contract, grep `README.md`, `AGENT.md`, and the PR description for the old shape before pushing. Mismatched docs cost ~3 review rounds on the maintenance-followups PR alone.
