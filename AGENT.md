...
Millpond has two independent credential paths that must not interfere (the lake-data one takes a different shape per destination):
...
- **S3 (hoglake data)**: `HOGLAKE_S3_ACCESS_KEY` / `HOGLAKE_S3_SECRET_KEY`, and they are optional as a PAIR. pyhoglake's `S3Config` passes each key to pyarrow's `S3FileSystem` only when it is not None, so with neither set pyarrow uses the AWS SDK default credential chain — which in Kubernetes is the ServiceAccount's web-identity token (IRSA), the model the hoglake server itself uses. Static keys stay for MinIO (local dev, CI). `config.load()` refuses one without the other and names both variables. pyarrow refuses half a pair too (`S3FileSystem(access_key=...)` with no secret raises `ValueError`), but only when pyhoglake first builds the filesystem and without naming either variable — the config check is about where and how the failure lands, not about a silent fallback that does not exist. `HOGLAKE_S3_ENDPOINT` and `HOGLAKE_S3_REGION` are independent of the key pair — the region is still needed under IRSA. The names are deliberately non-standard for the same reason the DuckLake ones are (see the paragraph below): under `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` they would take precedence in the chain and shadow IRSA for MSK IAM auth. With neither key set, the hoglake write path and MSK auth resolve the SAME IRSA identity, which is the intent.
...
`HoglakeSink.write` takes a second extra keyword, `trigger`. It is not part of the identity. It is why main.py flushed now (`size` / `time` / `final`), supplied through the same `write_kwargs` seam, and it reaches only the snapshot's commit message. Keeping it out of the key is deliberate — the same rows flushed for a different reason are the same rows, and a restart that reaches the size trigger where the dead process reached the time trigger must still be recognized as a replay.

Every hoglake commit records a `message` beside the `author`, built by `format_commit_message` and set on the payload in `_prepare` (beside `author`, so it is frozen with the payload and a replay re-sends the same text). The format is two parts:
...
Line 1 is a fixed sequence of `key=value` pairs in a fixed order, with no space inside any value (only the version can carry one, so `_message_token` replaces whitespace there). Line 2 lists every partition range sorted by partition number, names the topic once, and ends with the range count; a flush spanning two topics writes one such line per topic. THE OFFSET RANGES ARE THE REASON THE MESSAGE EXISTS: a snapshot's author names the pipeline and its files name object keys, and nothing else ties a published snapshot to a position in the topic — so without them nobody can answer "is offset X in the lake", audit a replay after a crash, or reconcile a gap without reading parquet. Rules for anything that touches it: the text must stay DETERMINISTIC for a given flush (the in-process retry re-sends the frozen payload, and a rebuild in a new process must produce the same text, so nothing wall-clock, random or per-attempt may enter it); it is FORMATTED BEFORE THE UPLOAD in `_prepare` and only attached to the payload afterwards, because everything that can raise after `prepare_append_files` must account for the objects it abandons (`_count_orphans`) and a formatting failure has no such arm — `test_a_formatter_failure_happens_before_any_upload` and a source-order assertion both pin that ordering; the offsets line is bounded at 16 KiB (`_MESSAGE_OFFSETS_LIMIT`), which holds about 780 ranges so that ONE pod owning a whole 512-partition topic (about 10.5 KiB) is never truncated, and over the bound it degrades to `... (+<k> more)` rather than growing, while the summary line is never truncated; `partitions` counts the partition tuples the fanout produced, which is 0 on an unpartitioned table and equals `files` under the one-file-per-tuple fanout (a `(None,)` tuple, which Iceberg gives a null source value, IS a partition and is counted); `files` is `len(groups)` and must not be read back off the payload, which would re-introduce a post-upload dependency; and `table=` is the destination incarnation (`self._table_uuid`), there for the same reason it is in the idempotency key — without it a range published into a dropped-and-recreated table produces a message byte-identical to its predecessor's. The trigger vocabulary in the message is `size` / `interval` / `final` / `unknown` while `metrics.batches_flushed_total` labels the interval trigger `time` — `_MESSAGE_TRIGGERS` maps the two, in one place, because a rename of the live metric label breaks the existing series and gives no benefit. `millpond=` comes from `cfg.service_version`, read ONCE at construction, so the message and the OTLP `service.version` resource attribute can never disagree about which build wrote a snapshot. `arrow_bytes=` is `batch.nbytes` as `write()` received it (before the unwritable columns are dropped and `_inserted_at` is stamped), which is APPROXIMATELY the flush gate's `pending_bytes` and not equal to it: the gate measures per accumulated table and the sink gets the consolidated one, so the concat, its type promotion and the sort move it — about +0.75% for a sorted flush, about +10% with schema drift. Do not document it as the gate value. The shutdown flush passes `final` explicitly (main.py's finally block), which also makes `batches_flushed_total{trigger="final"}` a series that did not exist before — it used to count as an ordinary interval flush.

`HoglakeSink.__init__` makes two network calls before the pod can claim readiness: it resolves the catalog, then probes object storage once (`_probe_object_store`) with a zero-byte upload of a fixed marker key, `<data_path>/_millpond/probe`, through the `S3FileSystem` pyhoglake builds. Zero-byte upload, not a single PutObject: pyarrow's `open_output_stream` opens a multipart upload eagerly, so closing an empty stream sends CreateMultipartUpload → UploadPart(1, 0 bytes) → CompleteMultipartUpload — one empty part, accepted by S3 and MinIO, all three calls authorised by `s3:PutObject` (`s3:AbortMultipartUpload` is not exercised on the success path; it covers an upload that dies half-way). An upload, not a LIST, for three independent reasons: it is the SAME `open_output_stream` call the flush path uploads parquet with (pyhoglake 1.3.0 `client.py:1051` in `prepare_append_files`, `_upload` at `:1353`), so it is mechanically predictive of the write path; the sink's role carries write access under the data path and NOT `ListBucket`, so a LIST tests the wrong grant; and pyarrow's `get_file_info(FileSelector(..., allow_not_found=True))` maps a `NoSuchBucket` 404 onto an empty listing, so a bucket typo in `HOGLAKE_DATA_PATH` passed the probe and failed on the first flush — after the catalog row had frozen the bad path, which hoglake has no route to undo (`allow_not_found=False` does not distinguish the two either: an empty prefix raises the same `FileNotFoundError`). The marker key is fixed, so each boot overwrites it; hoglake ignores objects it never registered (`cleanup` drains only server-queued paths, `verify` is metadata-only), so it is inert, not orphan debt. Failure is a permanent `HoglakeSinkError` naming the marker URI, the catalog, the credential source ("static HOGLAKE_S3_* keys" vs "the AWS default credential chain (IRSA in Kubernetes)"), the SDK's error text and the setting to check (the bucket on `NoSuchBucket` — the data path is frozen in the catalog row, so `HOGLAKE_DATA_PATH` only matters while the catalog is being created; `HOGLAKE_S3_REGION` / `AWS_REGION` on `AuthorizationHeaderMalformed` or `PermanentRedirect`; the static keys on `InvalidAccessKeyId` / `SignatureDoesNotMatch`); never retried, never a liveness check. The probe's success line is the single startup log of the credential source — never the key — and there is no metric. A pyhoglake that stops exposing the private filesystem accessor is a hard startup failure, not a skipped probe: a guard that stops guarding on an attribute rename still reads as verified.
...
- Pinned floor: the static config values are passed as `pinned` and are a PERMANENT part of the served set in authoritative mode (served set = polled ∪ static). Pins never enter the removal countdown (`current - remote - pinned` is the countdown domain) — exempt regardless of endpoint state, a pin the endpoint once served and later dropped stays. The `bulk_removal` guard is measured against the endpoint-MANAGED slice `current - (pinned - remote)`: endpoint-invisible pins are excluded (a large pin set must not mask a managed wipe) but endpoint-SERVED pins count (dropping 2 of its 12 must not read as 2-of-2 and refuse forever). A `bulk_removal` refusal still applies that poll's ADDITIONS (safe direction; starving a new team = silent drop with offsets committed) without committing countdown state — a genuine world-replacement stays refused because its removals remain a managed-slice majority, while genuine growth clears a disputed refusal. Empty-remote is ACCEPTED when the held set is pins-only (all-legacy deployment must sync, not crash-loop on startup timeout); refused as before when managed values are held. `__init__` rejects pins ⊄ bootstrap (a first-sight poll would bypass the type-flip guard and string-coerce the pins). Removing a pin is a config deploy, not an endpoint change. The shadow prober carries the same pins so its gauges predict the post-flip set; `include_values_pinned` / `include_values_pinned_only` survive the flip and are the authoritative-mode divergence signal (pins logged at startup for membership checks).
...
**Column type coercion (`MILLPOND_TYPED_COLUMNS`).** JSON carries no type schema, so inference can diverge from the destination column, and DuckLake's widening-only evolution then rejects the narrowing `ALTER` every flush (logged + `errors_total{type="schema"}`, swallowed per the per-column failure path above) while the write stalls under DuckLake at INSERT. Two cases on the duckling backfill's `posthog.events`: date-times infer `VARCHAR` vs a `TIMESTAMPTZ` column, and `project_id` (the one numeric column the producer serializes as explicit JSON `null` — no serde skip) infers `VARCHAR` in an all-null batch vs `BIGINT`. `arrow_converter.coerce_typed_columns()` (called from `main.py` right after `convert()`, gated on the env var) pins named columns to a target type (`timestamptz`/`bigint`/`double`/`boolean`/`varchar`/`uuid`, via the `_COERCERS` registry) *before* `evolve()`, so the inferred type already matches the table and no ALTER is issued; it also makes millpond-created tables use the right type from the start. `timestamptz` parses the ClickHouse-events wire string (space-separated, UTC implied, 0/3/6 fractional digits) via cast-to-naive + `assume_timezone` (PyArrow's `strptime`/`%f` doesn't parse it; a direct tz-aware cast demands a zone offset); other targets are plain casts. `uuid` is the one target whose Arrow type is an EXTENSION type: it emits `pa.uuid()` (pyarrow's canonical UUID extension over `fixed_size_binary(16)`) from an ENUMERATED set of input shapes — canonical 8-4-4-4-12, 32 bare hex, either wrapped in `{}` or prefixed with the case-sensitive `urn:uuid:` — which is deliberately NARROWER than `uuid.UUID`'s grammar (see `uuid_bytes` below). That wire form is deliberate and is the reason the target needs no destination rule — DuckDB reads `pa.uuid()` as a native `UUID` column and pyhoglake maps it to hoglake's `uuid`, while plain `pa.binary(16)` would land as a DuckDB BLOB from byte-identical data. (It would also carry no parquet `LogicalTypeAnnotation.uuidType()` of its own, but that no longer decides the hoglake path: `_prepare` casts to a schema naming `pa.uuid()` and the cast upgrades the bare form. DuckLake is the reason the coercer emits one type rather than one per destination.) `schema._ARROW_EXTENSION_TO_DUCKDB` is what keeps the DuckLake DDL side from narrowing it back to VARCHAR every flush. On the hoglake path the annotation reaches the file since the pyhoglake 1.3.0 pin: `_prepare` casts to `columns_to_arrow_schema(info.columns)`, which now names `pa.uuid()` for a uuid column, so the extension column passes through unchanged. 1.3.0 accepts both spellings on append, so a pod predating the pin still writes to the same table. Before it, that schema named `pa.binary(16)` and writing the annotated form anyway had the file refused (`parquet.schema_arrow.equals(columns_to_arrow_schema(...))` is an exact compare) — the two sides had to move together, hence a version floor rather than a millpond-side fix. 1.3.0 requires pyarrow >= 21, which is why millpond's pyarrow floor moved with it. Because the target is an extension type, `_coerce_or_null`'s per-value fallback rebuilds through the STORAGE type — an extension scalar's `as_py()` is the logical object (`uuid.UUID`), which `pa.array` cannot lay back out. The same extension-ness is why every stage BETWEEN the coercer and the sink reaches past the column to its `fixed_size_binary(16)` storage, because no pyarrow compute kernel accepts an extension type: `_apply_sort` sorts the storage via `_sortable_key_projection` (bytewise order == canonical text order) and gained an `unsortable_type` skip arm as the backstop; `_apply_filter`/`_apply_drop_filter` run `is_in` over the storage with the configured values parsed by `arrow_converter.uuid_bytes`, and `config._require_uuid_filter_values` refuses a non-UUID static filter value at load (an unevaluable allowlist drops every batch, an unevaluable denylist denies nothing — both silent). `uuid_bytes` enumerates its accepted shapes instead of delegating to `uuid.UUID`, whose grammar strips `urn:`/`uuid:` from anywhere and every hyphen from anywhere and so silently remaps non-UUIDs; variant/version bits are deliberately unvalidated (ClickHouse `UUID` is opaque 128 bits). Finally, `hoglake._rewrite_column` handles BOTH directions of the uuid/string mismatch (`_UUID_REWRITES`), because the pin has to be as safe to roll back as it is to apply. Live `string` + batch `uuid` is an operator ADDING the pin to a table created from unpinned batches; live `uuid` + batch `string` is an operator REMOVING it, or one pod on a mixed fleet that has not taken the new config. Neither casts — `cast(pa.uuid() -> string)` reinterprets the bytes as UTF-8 and raises `Invalid UTF8 payload`, `cast(string -> fixed_size_binary(16))` raises `widths must match` — and both are deterministic on the batch, so `is_retryable` correctly calls them permanent and the flush crashes on attempt 1 with offsets uncommitted. Hoglake has no `string <-> uuid` promotion either way, so the LIVE column wins: the batch column is rewritten to its shape, `errors_total{type="schema"}` fires every flush, the warning is once per column per direction, and text that is not a UUID is nulled with an additional `errors_total{type="column_coercion"}`. Note a rewrite NORMALIZES: of the five accepted input shapes only an already-canonical lowercase hyphenated string survives a round trip unchanged. `ducklake._align_uuid_columns` is the DuckLake half and covers the rollback direction only — live UUID + VARCHAR batch is parsed through `coerce_typed_columns` (the same code the pin runs, so there is one opinion about which strings are UUIDs) BEFORE `evolve()`, so no ALTER is issued and the insert is a typed append. It is not optional: DuckDB's implicit VARCHAR->UUID cast is all-or-nothing (`ConversionException` for the whole insert on one bad value, every retry) and refuses `urn:uuid:` which our grammar accepts. The other DuckLake direction needs nothing — DuckDB casts UUID->VARCHAR natively on INSERT, at the cost of one refused ALTER per flush. On the filter side an unparseable value is dropped from the comparison and the rest of the set still applies (`errors_total{type="filter_value_invalid"}`); only a set with none left falls back to the filter's direction, the allowlist under `records_skipped_total{reason="filter_value_invalid"}`. `is_retryable` now names `pa.ArrowInvalid | pa.ArrowTypeError | pa.ArrowNotImplementedError` explicitly rather than catching two of the three by their `ValueError`/`TypeError` bases. A column already the target type, or absent from the batch, is skipped. Non-fatal and type-consistent per column: a present, configured column is always emitted as the target type — values that can't be cast are **nulled** (only the unconvertible ones; good values kept), never left as the source type, so buffered batches stay concat-compatible at flush (`pa.concat_tables`) instead of raising `ArrowTypeError` outside the write-retry path. Failures bump `errors_total{type="column_coercion"}` (loud via metrics) and never raise on the consume path (a raise there would unwind past the offset bookkeeping). Clean coercions count via `columns_coerced_total{target_type=...}`.
...
| `pyarrow>=21.0` | Arrow tables, zero-copy DuckDB integration (floor set by pyhoglake 1.3.0) |
| `pyhoglake>=1.3.0` | Hoglake destination client; 1.3.0 is the floor so a uuid column's parquet carries the UUID logical annotation |
