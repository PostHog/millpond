...
HTTP exposition: `/metrics` (Prometheus), `/-/healthy` (k8s liveness — always 200 while the process answers), `/-/ready` (k8s readiness — 200 once the scheduler's running). The handler reads from a registry stored on the server object (`srv.registry`) rather than the default Prometheus registry, so integration tests can use an isolated `CollectorRegistry`. `_start_http(port, host="")` defaults to all-interfaces; tests pass `host="127.0.0.1"` because `HTTPServer.server_bind` calls `socket.getfqdn(host)` which on some macOS DNS configs takes 5 seconds for `""`.
...
**Timestamp coercion (`MILLPOND_TIMESTAMP_COLUMNS`).** JSON date-times infer as `VARCHAR`, which is the *new-table* default (hence "source `timestamp` fields are VARCHAR" above). When a consumer writes into a table whose timestamp columns are already `TIMESTAMPTZ` — the duckling backfill's `posthog.events` is the motivating case — that VARCHAR batch type can't reconcile: `TIMESTAMPTZ`→`VARCHAR` is a narrowing, so each flush's `ALTER COLUMN` is rejected (logged + `errors_total{type="schema"}`, swallowed per the per-column failure path above) and the write stalls under DuckLake at INSERT time. `arrow_converter.coerce_timestamp_columns()` (called from `main.py` right after `convert()`, gated on the env var) parses the named columns to `timestamp[us, tz=UTC]` *before* `evolve()`, so the inferred type already matches the table and no ALTER is issued. It also makes millpond-created tables use `TIMESTAMPTZ` from the start. Parse handles the ClickHouse-events wire string (space-separated, UTC implied, 0/3/6 fractional digits) via cast-to-naive + `assume_timezone` (PyArrow's `strptime`/`%f` doesn't parse it; a direct tz-aware cast demands a zone offset). Non-fatal per column: an unparseable value leaves that column as string for the batch and bumps `errors_total{type="timestamp_coercion"}` (loud via metrics, but never raises on the consume path — a raise there would unwind past the offset bookkeeping). Coercions count via `timestamp_columns_coerced_total`. Note this covers only timestamp columns; a non-timestamp narrowing like `project_id` (`BIGINT` in the events DDL, inferred `VARCHAR` in an all-null batch) is out of scope — see the README rollout caveat.
