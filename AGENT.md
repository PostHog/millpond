...
**Timestamp coercion (`MILLPOND_TIMESTAMP_COLUMNS`).** JSON date-times infer as `VARCHAR`, which is the *new-table* default (hence "source `timestamp` fields are VARCHAR" above). When a consumer writes into a table whose timestamp columns are already `TIMESTAMPTZ` — the duckling backfill's `posthog.events` is the motivating case — that VARCHAR batch type can't reconcile: `TIMESTAMPTZ`→`VARCHAR` is a narrowing the extension rejects, so the per-column failure path above triggers every flush and writes wedge. `arrow_converter.coerce_timestamp_columns()` (called from `main.py` right after `convert()`, gated on the env var) parses the named columns to `timestamp[us, tz=UTC]` *before* `evolve()`, so the inferred type already matches the table and no ALTER is issued. It also makes millpond-created tables use `TIMESTAMPTZ` from the start. Parse format is the ClickHouse-events wire string (`2024-01-01 12:00:00.000000`, UTC implied) via cast-to-naive + `assume_timezone` (PyArrow's `strptime`/`%f` doesn't parse it; a direct tz-aware cast demands a zone offset). Strict: an unparseable value raises rather than nulling, surfacing a producer format drift loudly. Counts via `timestamp_columns_coerced_total`.
