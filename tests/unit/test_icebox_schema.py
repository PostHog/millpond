...
def test_icebox_files_has_no_fk_to_commit_cycles():
    """v6 has no cycle abstraction. A regression that re-introduces an
    FK would couple the new code to the cycle table we're removing."""
    sql = ddl.CREATE_ICEBOX_FILES.lower()
    assert "references commit_cycles" not in sql


# ---------------------------------------------------------------------------
# IceboxPendingFileRow model
# ---------------------------------------------------------------------------
