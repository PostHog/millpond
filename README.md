...
**Orphaned objects are a real, unreclaimed cost.** Hoglake's `cleanup` reclaims only files the *server* queued for removal (snapshot expiry, table drop, compaction staging); files a client uploaded are not in that set, and automated cleanup for them is explicitly future work on the hoglake side. Every parquet millpond uploads without registering is therefore billed storage until somebody sweeps it. Two things make the sweep tractable: `millpond_hoglake_orphaned_files_total` counts the ones millpond can *prove* it orphaned, and every prepared upload lives under a `…/{idempotency_key}/` prefix of the table's data path, so a candidate object's liveness is decidable from the catalog.

The counter is exact for the paths it covers, and it covers every path where a *prepared payload exists* — so the files are known to be in object storage, and there are known to be `len(files)` of them: a commit the server refuses, a rebuilt flush the receipt declines, a payload superseded by the next flush, and a payload still unpublished at `close()`. Retries do not inflate it either, once there is a payload to hold: the retry re-sends that registration rather than re-uploading.

It deliberately counts **nothing** for a flush that fails *inside* `prepare`, and that is where its two blind spots are. Most such failures upload nothing at all, and the counter is simply right to stay still: pyhoglake validates file *i* before uploading file *i*, and every file in a millpond fanout is a slice of one aligned table (one schema, one partition arity, no empty groups), so a validation refusal is a property of the whole set and fires at index 0; and its catalog work — the key's UUID parse, the read-snapshot refresh, the incarnation pre-flight — all precedes the upload loop, so a 503, a read timeout or a 404 on a dropped table has written nothing either. The blind spot is the remaining case, an S3 failure genuinely partway through the fanout: pyhoglake does not report how far it got, so rather than book a number that is partly invented, millpond logs a warning naming the fanout width and the `{idempotency_key}/` prefix to sweep, and leaves the metric alone. A flush that keeps failing in `prepare` therefore re-uploads on every attempt and the counter stays flat — the log is the trace. The second blind spot is the orphan a `SIGKILL` leaves between the upload and the commit, which nothing can count.

The rule the counter is built to: it may miss an orphan, but every object it reports was written. The alternative — an upper bound — was worse than useless in practice, because it sent operators sweeping for objects that had never existed.
