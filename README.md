...
| Table dropped and recreated under the same name mid-flight | The key names the incarnation, so the new table never answers from the old one's receipt. The commit is refused, the payload dropped, and the flush rebuilds against the live incarnation. |
...
The counter covers every orphan path millpond can see: a prepare that fails partway through the partitioned fanout upload, a commit the server refuses, a rebuilt flush the receipt declines, a payload superseded by the next flush, and a payload still unpublished at `close()`. The partial-upload count is an upper bound (pyhoglake does not report how far it got) except for a single-file flush, where a validation refusal is known to have uploaded nothing. The one orphan nothing can count is the one a `SIGKILL` leaves between the upload and the commit.
