...
The counter covers every orphan path millpond can see: a prepare that fails partway through the partitioned fanout upload, a commit the server refuses, a rebuilt flush the receipt declines, a payload superseded by the next flush, and a payload still unpublished at `close()`. The partial-upload count is an upper bound, because pyhoglake does not report how far it got — with one thing known about it: pyhoglake validates file *i* and only then uploads it, so a *validation* refusal on an N-file fanout left at most N−1 objects behind, and on the single-file flush that is the common case, none at all. The one orphan nothing can count is the one a `SIGKILL` leaves between the upload and the commit.
