...
The hoglake path is stronger than that at the *commit* level, and it has to be. The catalog is reached over HTTP, so a commit the server applied whose response is lost looks exactly like a commit that never happened, and hoglake deliberately permits the same data-file path to be registered twice — a naive retry publishes the rows again. So each flush is published under an idempotency key derived from the Kafka offset range it covers, and the uploaded registration is held in memory across retries. The retry is then byte-identically the same request, and the server answers it from its receipt without writing. What that buys, precisely:

| Failure | Result |
|---|---|
| Commit response lost (timeout, reset, 502) | Retry replays the identical request; the rows publish **exactly once**. |
| Pod dies after the commit applied, before the offsets committed | Kafka replays the range; the rebuilt flush is recognized by its key and accepted without republishing (`millpond_hoglake_commit_replays_total{outcome="already_published"}`). The rebuilt upload is orphaned — see below. |
| Commit refused (409 conflict, 422 validation) | Zero rows registered, atomically. The uploaded parquet is orphaned. |
| Pod dies before the commit is sent | Nothing published; Kafka replays. The upload, if it happened, is orphaned. |

**Orphaned objects are a real, unreclaimed cost.** Hoglake's `cleanup` reclaims only files the *server* queued for removal (snapshot expiry, table drop, compaction staging); files a client uploaded are not in that set, and automated cleanup for them is explicitly future work on the hoglake side. Every parquet millpond uploads without registering is therefore billed storage until somebody sweeps it. Two things make the sweep tractable: `millpond_hoglake_orphaned_files_total` counts the ones millpond knows it orphaned, and every prepared upload lives under a `…/{idempotency_key}/` prefix of the table's data path, so a candidate object's liveness is decidable from the catalog. Retries do **not** add to this: a retry re-sends the registration rather than re-uploading.
